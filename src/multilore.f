...
C
 ...
 10   READ (1,1010,END=20,ERR=888) LABEL,IAT,AT1(IPT),RESNAM(IPT),
     $ RES1(IPT),XIN(1,IPT,I),XIN(2,IPT,I),XIN(3,IPT,I),P(IPT),B(IPT)
      IF (LABEL.EQ.'ENDM'.OR.LABEL.EQ.'END ') GOTO 20
C
C Select backbone atoms only
C
      WRITE(*,*) AT1(IPT),RESNAM(IPT),RES1(IPT)
      WRITE(*,*) BLANK, RESREF(IPT), ATREF(IPT)
      IF (AT1(IPT).NE.' N   '.AND.AT1(IPT).NE.' CA  '.AND.
     $    AT1(IPT).NE.' C   '.AND.AT1(IPT).NE.' O   '.AND.
     $    AT1(IPT).NE.' OT1 ') GOTO 10
C
...
C
 ...
 283  CONTINUE
      ...
      WRITE(21,1011) RES(IPEP),B(I),AVE1,RMS1,AVE2,RMS2,AVE12,RMS12,
     $                         dmin,jmin,imin 
 300  CONTINUE
      STOP
 ...
 1011 FORMAT(A4,F5.1,3(2F6.2,8x),f6.2,2I4)
